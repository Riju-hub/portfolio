...
function Skills() {
  return ( 
    ...
    >
      {/* Background Ambient Glows & Grid (Matching Hero & About) */}
      <div className='absolute inset-0 overflow-hidden pointer-events-none'>
      ...
      <div className='relative container mx-auto px-6 max-w-7xl z-10'>
        ...
        >
          ...
          <h2 className='text-3xl sm:text-4xl md:text-5xl font-black mb-4 tracking-tight text-white'>
            My <span className='text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-300'>Skills</span>
          </h2>
          <p className='text-neutral-400 max-w-2xl mx-auto text-base md:text-lg'>
            Technologies, frameworks, and tools I leverage to build modern digital products
          </p>
        </motion.div>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto'>
          {skills.map((skill, index) => (
            <motion.div
              ...
              viewport={{ once: true, amount: 0.2 }}
              className='relative bg-neutral-900/80 rounded-2xl p-6 md:p-7 border border-neutral-800/80 backdrop-blur-md hover:border-purple-500/40 hover:-translate-y-1.5 transition-all duration-300 cursor-pointer shadow-xl group overflow-hidden'
            >
              {/* Hover glow */}
              ...
              {/* Header */}
              <div className='relative flex items-center gap-4 mb-4'>
                <div className='w-14 h-14 rounded-xl bg-neutral-950 border border-neutral-800 flex items-center justify-center flex-shrink-0 group-hover:scale-110 group-hover:border-purple-500/30 transition-all duration-300'>
                  <skill.icon className={`${skill.color} w-7 h-7`} />
                </div>
                <h3 className='text-lg md:text-xl font-bold text-white group-hover:text-purple-300 transition-colors duration-300'>
                  {skill.title}
              ...
              {/* Description */}
              <p className='relative text-neutral-400 text-sm md:text-base mb-6 leading-relaxed'>
                {skill.description}
              </p>
