...
export default function ChatbotWidget() {
  ...
  // Lock background page scroll on mobile while chat is open
  useEffect(() => {
  ...
  return (
    <div className="font-sans select-none">
      ...
      {/* Trigger & Desktop Floating Position Anchor */}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] sm:inset-auto sm:bottom-6 sm:right-6 w-full h-[100dvh] sm:w-[385px] sm:max-w-[420px] sm:h-[min(550px,calc(100dvh-5.5rem))] flex flex-col pointer-events-auto"
          >
            <div className="bonfire-shadow-layer w-full h-full bg-slate-950/95 sm:bg-slate-950/25 border-0 sm:border border-sky-400/30 text-sky-100 rounded-none sm:rounded-3xl flex flex-col overflow-hidden backdrop-blur-xl relative">
              ...
              {/* Header */}
              <div className="relative z-10 bg-slate-900/60 sm:bg-slate-900/20 px-4 py-3.5 sm:py-3 flex items-center justify-between border-b border-sky-500/20 flex-shrink-0">
                ...
                {/* Glowing Rose Close Button */}
                <button
                  onClick={() => setIsOpen(false)}
                  className="relative group p-2 rounded-xl bg-gradient-to-tr from-pink-500/20 via-rose-500/25 to-fuchsia-500/20 hover:from-pink-500/35 hover:to-rose-500/40 border border-rose-400/40 hover:border-rose-400 text-rose-300 hover:text-rose-100 transition-all duration-300 active:scale-90 shadow-[0_0_12px_rgba(244,63,94,0.3)] hover:shadow-[0_0_16px_rgba(244,63,94,0.5)] cursor-pointer"
                  aria-label="Close chat"
                >
                    ...
                    <div
                      className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[13.5px] backdrop-blur-sm ${
                        msg.role === 'user'
                          ...
                          : 'bg-slate-900/80 sm:bg-slate-950/40 text-sky-50 rounded-tl-none border border-sky-500/25 shadow-[0_0_10px_rgba(0,0,0,0.2)]'
                      }`}
                    >
                      <FormattedMessage content={msg.content} />
                    </div>
              ...
              <div className="relative z-10 bg-slate-950/60 sm:bg-slate-950/20 border-t border-sky-500/15 px-3 py-2.5 flex items-center gap-2 overflow-x-auto hide-scrollbar flex-shrink-0">
              ...
              <form
                onSubmit={handleSubmit}
                className="relative z-10 p-3 pb-4 sm:pb-3 bg-slate-900/90 sm:bg-gradient-to-t sm:from-slate-950/70 sm:via-slate-900/35 sm:to-transparent border-t border-sky-500/20 flex items-center space-x-2 flex-shrink-0"
              >
                <div className="relative flex-1 group">
                  <div className="absolute -inset-0.5 rounded-xl bg-gradient-to-r from-sky-500 via-cyan-400 to-indigo-500 opacity-25 group-hover:opacity-60 group-focus-within:opacity-100 transition-all duration-300 blur-[2px]" />
                  ...
                  <div className="relative flex items-center bg-slate-950/90 sm:bg-slate-950/75 backdrop-blur-md rounded-xl border border-sky-400/30 group-focus-within:border-sky-300 transition-all duration-300 shadow-[inset_0_1px_4px_rgba(0,0,0,0.5)]">
                    ...
                    {/* text-[16px] prevents iOS Safari auto-zoom on mobile keyboards */}
                    <input
                      ref={inputRef}
                      type="text"
                      value={input}
                      ...
                      className="w-full bg-transparent px-3 py-3 sm:py-2.5 text-[16px] sm:text-sm text-sky-100 placeholder-sky-300/40 focus:outline-none font-mono selection:bg-cyan-500/30 selection:text-white"
                    />
                  </div>
                </div>
